...
# calculating p's and pi's for 100 different initialization
num_of_clusters = 4
all_ps = []  # 100 elements, one element is a list(4 scaler elements)
...
for i in range(100):
    ...
    p = []
    dim = data_arr.shape[1]

    for key in init_clusters:
        numOfPoints = len(init_clusters[key])

        sum_vec = [sum(x) for x in init_clusters[key]]
        p.append(sum(sum_vec) / (numOfPoints * dim))

    all_ps.append(p)

# calculating log-likelihood values
def logLiklihood(data_arr, pi, p, numOfClusters):
    dim = data_arr.shape[1]
    ...
    for i in range(data_arr.shape[0]):
        ...
        numOfOnes = sum(data_arr[i])
        sum_val = 0

        for k in range(numOfClusters):
            sum_val += (pi[k] * (p[k] ** numOfOnes) * ((1 - p[k]) ** (dim - numOfOnes)))

        final_sum += np.log(sum_val)
...
for iter in range(100):
    ...
    new_log_liklihood = logLiklihood(data_arr, all_pis[iter], all_ps[iter], numOfMixtures)
    ...
    while diff_val > 0.0001:
        ...
        for i in range(data_arr.shape[0]):
            ...
            for k in range(numOfMixtures):
                ...
                lambda_mat[i][k] = (all_pis[iter][k] * (all_ps[iter][k] ** numOfOnes) * (
                            (1 - all_ps[iter][k]) ** (dim - numOfOnes)))

                sum_list = []
                for l in range(numOfMixtures):
                    sum_list.append(all_pis[iter][l] * (all_ps[iter][l] ** numOfOnes) * (
                                (1 - all_ps[iter][l]) ** (dim - numOfOnes)))

                sum_val = sum(sum_list)
                # print("sum before lambda : ", sum_val)
        ...
        for k in range(numOfMixtures):
            sum_val = 0
            for i in range(data_arr.shape[0]):
                sum_val += (lambda_mat[i][k] * (sum(data_arr[i]) / dim))

            all_ps[iter][k] = sum_val / N[k]
        ...
        diff_val = abs(new_log_liklihood - old_log_liklihood)
...
def predict(X, numOfMixtures):
    ...
    for n in range(len(X)):
        numOfOnes = sum(data_arr[n])
        probas.append(
            [(all_ps[idx][k] ** numOfOnes) * ((1 - all_ps[idx][k]) ** (dim - numOfOnes)) for k in range(numOfMixtures)])
